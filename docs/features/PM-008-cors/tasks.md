...
- [x] B-010: Run `pnpm build` and confirm it compiles clean with the new `express`/`cors`/`@as-integrations/express5` types in place.
- [x] B-011: Update `README.md` (if it documents how to start the server / hit the API locally) to mention `ALLOWED_ORIGINS` must be set for a browser-based client to work.
