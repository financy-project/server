...
- [x] T-019: `categoriesById` loader batches and returns results in input-key order, `null` for missing ids
- [x] T-020: `createTransaction` mutation (e2e): happy path; foreign `categoryId` → `NOT_FOUND`; invalid `value`/`type`/`date`/`description` → `BAD_USER_INPUT`; unauthenticated → `UNAUTHENTICATED`
- [x] T-021: `listTransactions` query (e2e): returns only the caller's own transactions, respects date filters, defaults to the current month, paginates, resolves `category`
- [x] T-022: `updateTransaction`/`deleteTransaction` mutations (e2e): happy path; another user's transaction → `NOT_FOUND`; nonexistent id → `NOT_FOUND`; foreign `categoryId` on update → `NOT_FOUND`
- [x] T-023: Deleting a category unlinks (not deletes) its transactions — `category: null` on re-query, transaction still present in `listTransactions`
