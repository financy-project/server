...
> **Flat architecture, project-wide.** Every domain (`auth`, `user`, `health`, `category`, `transaction`, `dashboard`) was flattened away from the module-based DDD pattern described in `docs/architecture/` (no more `src/modules/<name>/`, no ports/adapters/gateways, no use-case layer) — see **[constitution.md § Flat Architecture](constitution.md#flat-architecture)** for the current structure and why. `docs/architecture/` and `/scaffold` now describe a **retired pattern**, kept for historical reference only. Everything below that references modules, `/scaffold`, or the backend checklist's layer order is stale — treat constitution.md's Flat Architecture section as the source of truth instead.
